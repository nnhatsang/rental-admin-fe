...
export const requestCreateAssetUnit = (
  data: ICreateAssetUnitReq,
...
export const requestDeleteAssetUnits = (
  assetUnitIds: string[],
): Promise<AxiosResponse<DefaultResponse<IAssetUnitActionRes>>> => {
  const config: AxiosRequestConfig = { method: 'DELETE', url, data: { assetUnitIds } };
  return apiAuth(config);
};
