...
import { USER_ACTIVITY_STATUSES } from '@/utils/consts/resource-status.const';
import * as z from 'zod';

const roleCodeSchema = z.enum(ROLE_CODES as [string, ...string[]]);
const userActivityStatusSchema = z.enum(USER_ACTIVITY_STATUSES as [string, ...string[]]);
...
export const resetUserPasswordSchema = z
  .object({
    newPassword: z
      .string()
      .regex(/^(?=.*[a-z\d])(?=.*[A-Z\d])(?=.*\d)(?=.*[@$!%*?&><])[A-Za-z\d@$!%*?&><]{8,32}$/, {
